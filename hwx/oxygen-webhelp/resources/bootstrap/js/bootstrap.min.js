...
/*Removes the Table of Contents link from breadcrumbs*/
  $("ol.hidden-print li:contains('Table of Contents')").remove();

/*Removes the Table of Contents text from side toc*/
  $("ul.hidden-print li span:contains('Table of Contents')").remove();

/*Removes the hrefs with hidetoc from side toc*/
  $('.wh_side_toc li span a[href*="hidetoc"]').parent().parent().remove();

//find all href in the .wh_tile_title class that contains _hidetoc and remove it from href
$('.wh_tile_title a').each(function() { 
     var $this = $(this),
...
$(document).ready(function() {
    ...
    return $('<sup/>', {
   	...
   	//Change Equation-Figures prefix for Title and Xref from Figure to EQ.
...
$(".wh_publication_title a .title:contains('RADIOSS')").each(function() {
    var all = document.getElementsByClassName('varname');
...
//Changes the varname font style from italic to normal for RADIOSS
$(".wh_publication_title a .title:contains('RADIOSS')").each(function() {
    var all = document.getElementsByClassName('varname');
	for (var i = 0; i < all.length; i++) {
  	all[i].style.fontStyle = "normal";
...
for(var tclass=0;tclass<x.length;tclass++){
	var xtest=x[tclass].parentNode.parentNode.getElementsByTagName("h2");
	if (xtest[0].innerHTML=="Definitions"){
		var xsub=x[tclass].getElementsByTagName("col");
		xsub[0].style.width="20%";
		xsub[1].style.width="65%";
	}
};
...
//Adds div to create seperator after the first three tiles on the homepage
$('.wh_tile:nth-child(3)').after('<div class="seperator"></div>');